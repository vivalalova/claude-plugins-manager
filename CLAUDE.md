...
| Section | 渲染模式 | 涵蓋欄位 |
| --- | --- | --- |
| GeneralSection | **全 schema-driven**（`getSectionFieldOrder('general')` loop）；支援 `nestedUnder`（如 `defaultMode` 位於 `permissions.defaultMode`） | defaultMode（nestedUnder permissions）、effortLevel、language、availableModels、includeGitInstructions、respectGitignore、fastMode、fastModePerSessionOptIn、autoMemoryEnabled、outputStyle、autoUpdatesChannel、cleanupPeriodDays |
