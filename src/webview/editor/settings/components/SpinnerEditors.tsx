...
// ---------------------------------------------------------------------------
// Spinner editors — object-typed display fields (spinnerVerbs / spinnerTipsOverride).
// Consumed only by ObjectFieldEditor; kept separate so the dispatcher does not
// import DisplaySection (which would create a cycle once DisplaySection consumes
// the dispatcher).
// ---------------------------------------------------------------------------
...
export function SpinnerVerbsEditor({ scope, value, onSave, onDelete }: SpinnerVerbsEditorProps): React.ReactElement {
  const { t } = useI18n();
...
export function SpinnerTipsOverrideEditor({ scope, value, onSave, onDelete }: SpinnerTipsOverrideEditorProps): React.ReactElement {
  const { t } = useI18n();
