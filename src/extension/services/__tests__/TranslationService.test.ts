...
describe('TranslationService', () => {
  ...
  describe('translate — 基本行為', () => {
    ...
    it('去重：相同文字只翻譯一次', async () => {
      ...
      expect(result).toEqual({ 'same text': '翻譯' });
    });

    it('空陣列不呼叫 API', async () => {
      const result = await service.translate([], 'zh-TW');
  ...
  describe('translate — cache', () => {
    it('已 cache 的文字不再呼叫 API', async () => {
      ...
      // 第二次呼叫：應該從 cache 讀取
      const result = await service.translate(['text A'], 'zh-TW');

      expect(mockHttpsRequest).not.toHaveBeenCalled();
      expect(result).toEqual({ 'text A': '翻譯A' });
    });
    ...
    it('不同語言分開 cache', async () => {
      ...
      const result = await service.translate(['text'], 'ja');
    ...
    it('載入既有 cache 檔案', async () => {
      ...
      const result = await svc.translate(['other text'], 'zh-TW');

      // 應該只翻譯 other text，不含 cached hello
      expect(mockHttpsRequest).toHaveBeenCalledTimes(1);
      expect(result).toEqual({ 'other text': '新翻譯' });
    });
  });
  ...
  describe('translate — API 回應解析', () => {
    it('API 回傳多餘空白仍能正確解析', async () => {
      ...
      const result = await service.translate(['a', 'b'], 'zh-TW');

      expect(result).toEqual({ a: '翻譯A', b: '翻譯B' });
    });
  ...
  describe('translate — API 錯誤', () => {
    it('API 錯誤時 throw', async () => {
      ...
      await expect(service.translate(['text'], 'zh-TW'))
        .rejects.toThrow('network error');
    });

    it('API 回傳非 200 status 時 throw', async () => {
      mockApiResponse('', 429);

      await expect(service.translate(['text'], 'zh-TW'))
        .rejects.toThrow('API status: 429');
    });
  });
});
